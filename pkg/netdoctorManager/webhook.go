...
func (s *webhookhander) Default(ctx context.Context, obj runtime.Object) error {
	...
	finalizerName := "spiderdoctor.spidernet.io"
...
func (s *webhookhander) ValidateUpdate(ctx context.Context, oldObj, newObj runtime.Object) error {
	...
	old, ok := oldObj.(*crd.Netdoctor)
	if !ok {
		s := "failed to get oldObj"
		logger.Error(s)
		return apierrors.NewBadRequest(s)
	}
	new, ok := newObj.(*crd.Netdoctor)
	if !ok {
...
func (s *netdoctorManager) RunWebhookServer(webhookPort int, tlsDir string) {
	logger := s.logger
	r := &webhookhander{
		logger: logger,
	}
